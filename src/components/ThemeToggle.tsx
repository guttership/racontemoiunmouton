...
export default function ThemeToggle() {
  ...
  const themes: Array<{ value: 'light' | 'dark' | 'system'; icon: React.ReactNode; label: string }> = [
    { value: 'light', icon: <Sun className="h-5 w-5" />, label: t('light') },
    { value: 'dark', icon: <Moon className="h-5 w-5" />, label: t('dark') },
    { value: 'system', icon: <Monitor className="h-5 w-5" />, label: t('system') },
  ];
